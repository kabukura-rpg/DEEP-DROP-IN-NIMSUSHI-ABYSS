...
describe('falling and ammunition', () => {
  ...
  it('uses a round and brakes the fall without creating upward motion', () => { const game = emptyGame(); game.player.vy = 100; game.shoot(); expect(game.player.vy).toBe(0); expect(game.ammo).toBe(5); expect(game.bullets).toHaveLength(1); });
  it('limits shots by ammunition with no regeneration in the air', () => { const game = emptyGame(); for (let i = 0; i < 8; i++) { game.cooldown = 0; game.shoot(); } expect(game.ammo).toBe(0); expect(game.bullets).toHaveLength(6); tick(game, 0.2); expect(game.ammo).toBe(0); });
  it('refills and ends combo only on a landing from above', () => { const game = emptyGame(); game.platforms = [{ id: 4, x: 150, width: 150, y: 260 }]; game.ammo = 1; game.combo = 5; tick(game, 0.5); expect(game.player.y).toBe(245); expect(game.player.grounded).toBe(4); expect(game.ammo).toBe(6); expect(game.combo).toBe(0); const count = game.events.filter(e => e.type === 'land').length; tick(game, 0.2); expect(game.events.filter(e => e.type === 'land')).toHaveLength(count); });
...
describe('combat', () => {
  ...
  it('stomps a normal enemy, creates a bounce, and keeps ammo unchanged', () => { const game = emptyGame(); game.player.y = 260; game.player.vy = 300; game.enemies = [enemy('slime')]; tick(game, 0.05); expect(game.kills).toBe(1); expect(game.player.vy).toBeLessThan(0); expect(game.ammo).toBe(6); expect(game.hp).toBe(4); });
  ...
  it('pierces multiple enemies once each with one round', () => { const game = emptyGame(); game.stats.piercing = true; game.enemies = [enemy('slime', 225, 260, 1), enemy('slime', 225, 310, 2)]; game.shoot(); tick(game, 0.2); expect(game.kills).toBe(2); expect(game.ammo).toBe(5); });
