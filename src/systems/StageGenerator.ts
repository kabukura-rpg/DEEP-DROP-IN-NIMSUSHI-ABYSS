...
interface RowTuning { minWidth: number; maxWidth: number; gap: number; enemyChance: number; flyChance: number; toughChance: number; heavyChance: number; comboBias: number; bubbleChance: number; airPocketChance: number; maxOxygenGap: number; bubbleOffside: number; lavaPoolChance: number; lavaWallChance: number; ventChance: number; iceChance: number; iceOffside: number; breakableChance: number; maxBreakableRun: number }
...
export class StageGenerator {
  ...
  private tuningAt(localDepth: number): RowTuning {
    ...
    if (plan) {
      ...
      return {
        ...
        bubbleChance: plan.bubbleChance ?? 0, airPocketChance: plan.airPocketChance ?? 0,
  ...
  chunk(index: number): { platforms: RoutePlatform[]; enemies: Enemy[]; pickups: Pickup[]; airPockets: AirPocket[]; hazards: Hazard[] } {
    const platforms: RoutePlatform[] = [], enemies: Enemy[] = [], pickups: Pickup[] = [], airPockets: AirPocket[] = [], hazards: Hazard[] = [];
    ...
    return { platforms, enemies, pickups, airPockets, hazards };
  }
   ...
   */
  private placeAir(tuning: RowTuning, platform: RoutePlatform, y: number, start: number, pickups: Pickup[], airPockets: AirPocket[], enemies: Enemy[]) {
    ...
    if (!overdue && this.random() >= tuning.bubbleChance + tuning.airPocketChance) return;
    ...
    if (this.random() < tuning.airPocketChance / (tuning.bubbleChance + tuning.airPocketChance)) {
    ...
    let x = clamp(exit + lean * tuning.bubbleOffside * reach, 20);
    // Never bury a bubble inside an enemy's patrol.
    for (const e of enemies) if (Math.abs(e.y - bandY) < 40 && Math.abs(e.originX - x) < e.range + 30) x = clamp(x + (e.originX > x ? -1 : 1) * (e.range + 34), 20);
    if (y >= start) pickups.push(spawnPickup('oxygenBubble', this.id++, Math.round(x), Math.round(bandY), this.random() * Math.PI * 2));
    this.lastAirY = bandY;
  }
