...
<main class="layout">
  ...
  <aside class="guide"><div class="guide-title"><span>FIELD GUIDE</span><span>01—03</span></div><section class="guide-item"><span class="guide-index">01</span><h2>落ちる。狙う。</h2><p>左右に動いて、ルートを選ぶ。<br>ジャンプはできない。</p><div class="key-row"><kbd>A</kbd><kbd>D</kbd><span>or</span><kbd>←</kbd><kbd>→</kbd></div></section><section class="guide-item"><span class="guide-index">02</span><h2>撃って、ブレーキ。</h2><p>真下へ射撃。反動で減速。<br>空中の弾数は、たった6発。</p><div class="key-row"><kbd class="space-key">SPACE</kbd><span>長押しで連射</span></div></section><section class="guide-item"><span class="guide-index">03</span><h2>着地で、もう一度。</h2><p>足場に降りれば全弾リロード。<br>着地せず倒せばコンボが続く。</p><div class="reload-demo"><span>▰ ▰ ▰ ▰ ▰ ▰</span><small>FULL RELOAD ↺</small></div></section><div class="tip"><span>↳ KEEP IN MIND</span><p>トゲのある敵は踏まないこと。<br>休憩地点で強化を1つ選ぶ。<br>自動回復なし。25コンボでHP +1。</p></div><div class="guide-footer"><kbd>ESC</kbd><span>ひと休みする</span></div></aside>
...
function updateHud(model: GameModel) {
  ...
  const key = [Math.floor(model.sectionDepth), model.stage.label, model.oxygen.enabled ? model.oxygen.remaining.toFixed(1) : '-', model.sheltered, model.heat.enabled ? model.heat.value.toFixed(1) : '-', model.boss.enabled ? `${Math.ceil(model.boss.ratio * 100)}|${model.boss.phaseId}` : '-', model.hp, model.health.overflowHealing, model.stats.maxHp, model.ammo, model.stats.maxAmmo, model.gun.id, model.combo, model.multiplier, model.practice].join('|');
