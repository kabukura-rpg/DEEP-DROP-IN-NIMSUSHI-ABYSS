...
/** The widest. A gate's drop is the longest fall the AREA asks for, and it stays under a screen. */
export const CATACOMB_MAX_STEP = 300;
  ...
  return [
    {
      ...
      rows: r => Array.from({ length: count(r, 1, 2) }, () => ({
        piece: 'normal' as const, step: between(r, 196, 236), widthBias: [0.4, 1] as const,
    ...
    },
    {
      ...
      rows: r => Array.from({ length: count(r, 1, 2) }, () => ({
        piece: 'slot' as const, step: between(r, 188, 228), widthBias: [0, 1] as const,
        slot: t.slotGap, spikeChance: t.slotSpike, extras: 0, openSpan: false,
      })),
    },
    {
      // The gate row the SECTION owed, then a short drop -- the AREA's longest -- then a wide catch.
      id: 'breakableDrop', weight: 0.1,
      rows: r => [
        { piece: 'breakableDrop', step: between(r, 250, CATACOMB_MAX_STEP), widthBias: [0, 1], extras: 0, openSpan: true },
        { piece: 'breakableDrop', step: between(r, 196, 230), widthBias: [0.6, 1], extras: 0, openSpan: false },
      ],
    },
  ];
}
