...
errors.length=0;const results={};
for(const [name,width,height,overlay,alignment,resolution] of ['128','256'].flatMap(r=>['A','B'].flatMap(a=>[[r+'-'+a+'-desktop',1280,900,false,a,r],[r+'-'+a+'-mobile',390,844,false,a,r],[r+'-'+a+'-overlay',1280,900,true,a,r]]))){ 
 await cdp('Emulation.setDeviceMetricsOverride',{width,height,deviceScaleFactor:1,mobile:false});
 await cdp('Page.navigate',{url:'http://127.0.0.1:5197/DEEP-DROP-IN-NIMSUSHI-ABYSS/output/nimushi-boss-art-v1/resolution-review-v1/preview.html'+'?resolution='+resolution+'&alignment='+alignment+(overlay?'&overlay=1':'')});
 await until('!!window.review');await sleep(300);
 results[name]=await ev('window.review');const shot=await cdp('Page.captureScreenshot');await fs.writeFile(path.join(root,name+'.png'),Buffer.from(shot.data,'base64'));
}
await fs.writeFile(path.join(root,'browser-results.json'),JSON.stringify({results,errors},null,2));console.log(JSON.stringify({results,errors}));ws.close();
