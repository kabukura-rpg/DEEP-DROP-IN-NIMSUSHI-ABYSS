...
im=Image.open(R/'MASTER_NIMUSHI.png').convert('RGBA');a=im.getchannel('A').point(lambda v:255 if v>=128 else 0);im.putalpha(a);box=a.getbbox();crop=im.crop(box)
...
meta.update(status='NIMUSHI CHARACTER DESIGN HUMAN APPROVED / LOCKED; GAMEPLAY ALIGNMENT PENDING',alignments={'A':{'renderOffsetY':0},'B':{'renderOffsetY':-16}},masterFile='MASTER_NIMUSHI.png')
import hashlib
meta['masterSha256']=hashlib.sha256((R/'MASTER_NIMUSHI.png').read_bytes()).hexdigest()
meta['spriteSha256']=hashlib.sha256((R/'nimushi_idle_lock_v1.png').read_bytes()).hexdigest()
