...
const previewBefore=await ev('JSON.stringify(__dev.scene.model)');
for(const pose of ['idle','run','jump_rise','fall','gunboots_fire','gunboots_brake','landing','wall_contact','wall_kick','damage','death','boss_ascend','boss_brake','boss_hover','boss_damage']){
 await ev('__playerArt("prototype",1,'+JSON.stringify(pose)+')');await sleep(100);await shot('visual-only-'+pose);overrideResults.push(await state());
}
await ev('__playerArt("prototype",1,"auto")');
const previewUnchanged=previewBefore===await ev('JSON.stringify(__dev.scene.model)');
const result={overrideResults,previewUnchanged,method:'Real CDP keyboard/mouse inputs. Existing BOSS TEST entry used, no model modifications. Existing pause freezes snapshots; no compositing.',grounded,normal,bossEntry,ascend,brake,pausedBrake,left,scale2,abModelUnchanged:abBefore===abAfter,scaleModelUnchanged:beforeScale===afterScale,layerOrder,assets,errors};
